...
def train_resnet50(datasets_parameters, data_type, results):
    ...
    if f'resnet50_{data_type}_train_accuracy' not in results.columns or TRAIN_NEW_MODELS:
        print('Never trained this model')
        results[f'resnet50_{data_type}_train_accuracy'] = np.full((len(results)), -1).tolist()
        results[f'resnet50_{data_type}_val_accuracy'] = np.full((len(results)), -1).tolist()
        results[f'resnet50_{data_type}_test_accuracy'] = np.full((len(results)), -1).tolist()
        ...
        if dataset["name"] not in DATASETS_WITH_BAD_IMAGE_RESOLUTIONS:
            ...
            if results.iloc[i][f'resnet50_{data_type}_train_accuracy'] == -1:
                ...
                if has_val_dataset:
                    ...
                    results.loc[i,f'resnet50_{data_type}_val_accuracy'] = val_accuracy
...
def train_resnet101(datasets_parameters, data_type, results):
    # transformers
    train_transform = transforms.Compose([lambda x: black_and_white_image_to_binary(x), transforms.ToTensor()])
    val_transform = transforms.Compose([lambda x: black_and_white_image_to_binary(x), transforms.ToTensor()])
    test_transform = transforms.Compose([lambda x: black_and_white_image_to_binary(x), transforms.ToTensor()])
    if data_type == 'line_plots':
        ...
        test_transform = transforms.Compose([lambda x: plotting_time_normalization(x), transforms.ToTensor()])
    # add column for model results
    if f'resnet101_{data_type}_train_accuracy' not in results.columns or TRAIN_NEW_MODELS:
        print('Never trained this model')
        results[f'resnet101_{data_type}_train_accuracy'] = np.full((len(results)), -1).tolist()
        results[f'resnet101_{data_type}_val_accuracy'] = np.full((len(results)), -1).tolist()
        results[f'resnet101_{data_type}_test_accuracy'] = np.full((len(results)), -1).tolist()
        results.to_csv(RESULTS, index=False)
    model_parameters = pd.read_excel(open('datasets_and_model_parameters.xlsx', 'rb'), sheet_name='resnet_parameters')
    # training model
    for i, dataset in datasets_parameters.iterrows():
        # model parameters
        num_epochs = int(model_parameters.iloc[i]['resnet101_epochs'])
        batch_size = int(model_parameters.iloc[i]['resnet101_batch_size'])
        learning_rate = float(model_parameters.iloc[i]['resnet101_learning_rate'])
        early_stop_patience = int(model_parameters.iloc[i]['resnet101_early_stop_patience'])
        early_stop_callback = EarlyStopping(monitor='val_loss', min_delta=.0, patience=early_stop_patience, verbose=True, mode='min')
        print(f'[{i + 1}/{len(datasets_parameters)}] RESNET101 {dataset["name"]}: num_epochs = {num_epochs}, batch_size = {batch_size}, learning_rate = {learning_rate}, early_stop_patience = {early_stop_patience}')
        if dataset["name"] not in DATASETS_WITH_BAD_IMAGE_RESOLUTIONS:
            dataset_path = os.path.join(DATASETS_PATH, dataset['name'], data_type)
            train_dataset = None
            test_dataset = None
            ...
            model = None
            trainer = Trainer(callbacks=[early_stop_callback], gpus=GPUS, max_epochs=num_epochs, fast_dev_run=FAST_DEV_RUN)
            if results.iloc[i][f'resnet101_{data_type}_train_accuracy'] == -1:
                ...
                results.loc[i,f'resnet101_{data_type}_train_accuracy'] = train_accuracy
                results.loc[i,f'resnet101_{data_type}_test_accuracy'] = test_accuracy
                if has_val_dataset:
                    val_accuracy = trainer.test(model, val_loader)[0]['test_accuracy']
                    results.loc[i,f'resnet101_{data_type}_val_accuracy'] = val_accuracy
                    print(f'train_accuracy = {train_accuracy}, test_accuracy = {test_accuracy}, val_accuracy = {val_accuracy}')
                else:
                    print(f'train_accuracy = {train_accuracy}, test_accuracy = {test_accuracy}')
                results.to_csv(RESULTS, index=False)
...
def train_resnet152(datasets_parameters, data_type, results):
    # transformers
    train_transform = transforms.Compose([lambda x: black_and_white_image_to_binary(x), transforms.ToTensor()])
    val_transform = transforms.Compose([lambda x: black_and_white_image_to_binary(x), transforms.ToTensor()])
    test_transform = transforms.Compose([lambda x: black_and_white_image_to_binary(x), transforms.ToTensor()])
    if data_type == 'line_plots':
        ...
        test_transform = transforms.Compose([lambda x: plotting_time_normalization(x), transforms.ToTensor()])
    # add column for model results
    if f'resnet152_{data_type}_train_accuracy' not in results.columns or TRAIN_NEW_MODELS:
        print('Never trained this model')
        results[f'resnet152_{data_type}_train_accuracy'] = np.full((len(results)), -1).tolist()
        results[f'resnet152_{data_type}_val_accuracy'] = np.full((len(results)), -1).tolist()
        results[f'resnet152_{data_type}_test_accuracy'] = np.full((len(results)), -1).tolist()
        results.to_csv(RESULTS, index=False)
    model_parameters = pd.read_excel(open('datasets_and_model_parameters.xlsx', 'rb'), sheet_name='resnet_parameters')
    # training model
    for i, dataset in datasets_parameters.iterrows():
        # model parameters
        num_epochs = int(model_parameters.iloc[i]['resnet152_epochs'])
        batch_size = int(model_parameters.iloc[i]['resnet152_batch_size'])
        learning_rate = float(model_parameters.iloc[i]['resnet152_learning_rate'])
        early_stop_patience = int(model_parameters.iloc[i]['resnet152_early_stop_patience'])
        early_stop_callback = EarlyStopping(monitor='val_loss', min_delta=.0, patience=early_stop_patience, verbose=True, mode='min')
        print(f'[{i + 1}/{len(datasets_parameters)}] RESNET152 {dataset["name"]}: num_epochs = {num_epochs}, batch_size = {batch_size}, learning_rate = {learning_rate}, early_stop_patience = {early_stop_patience}')
        if dataset["name"] not in DATASETS_WITH_BAD_IMAGE_RESOLUTIONS:
            dataset_path = os.path.join(DATASETS_PATH, dataset['name'], data_type)
            train_dataset = None
            test_dataset = None
            ...
            model = None
            trainer = Trainer(callbacks=[early_stop_callback], gpus=GPUS, max_epochs=num_epochs, fast_dev_run=FAST_DEV_RUN)
            if results.iloc[i][f'resnet152_{data_type}_train_accuracy'] == -1:
                print(dataset['num_classes'])
                ...
                train_accuracy = trainer.test(model, train_loader)[0]['test_accuracy']
                test_accuracy = trainer.test(model, train_loader)[0]['test_accuracy']
                results.loc[i,f'resnet152_{data_type}_train_accuracy'] = train_accuracy
                results.loc[i,f'resnet152_{data_type}_test_accuracy'] = test_accuracy
                if has_val_dataset:
                    val_accuracy = trainer.test(model, val_loader)[0]['test_accuracy']
                    results.loc[i,f'resnet152_{data_type}_val_accuracy'] = val_accuracy
                    print(f'train_accuracy = {train_accuracy}, test_accuracy = {test_accuracy}, val_accuracy = {val_accuracy}')
                else:
                    print(f'train_accuracy = {train_accuracy}, test_accuracy = {test_accuracy}')
                results.to_csv(RESULTS, index=False)
